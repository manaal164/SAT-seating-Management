...
// ✅ NEW: GET student by roll_number (with joined seat info)
router.get("/:roll_number", (req, res) => {
  const roll = req.params.roll_number;

  const sql = `
    SELECT 
      st.id, st.roll_number, st.full_name, st.created_at,
      s.seat_number, s.is_booked, s.booked_at
    FROM students st
    LEFT JOIN seats s 
      ON s.booked_by_roll = st.roll_number AND s.is_booked = 1
